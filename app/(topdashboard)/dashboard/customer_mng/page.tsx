 ...
 *
 * Purpose:
 * - Mirrors the OrdersPage structure you provided but for Customer Management.
 * - 8 columns: Name, Email, Order (count), Total Spent, City, Last Order, Status, Action.
 * - First 3 columns (Name, Email, Order) together consume ~40% of the table width.
 *
 * Notes:
 * - This file includes tiny inline modal components for "View customer" and "Status actions"
 *   so the page is self-contained (no external components required).
 * - Keep styling consistent with your OrdersPage (Tailwind classes).
 */
...
type Customer = {
  ...
  totalSpent: string;
  city: string;
  lastOrder: string; // ISO date or display string
...
const CustomersPage: React.FC = () => {
  ...
  // FILTER CONFIG: reusable, similar to OrdersPage
  ...
  // Helper: badge classes by status (keeps table markup clean)
  ...
  // Optional: search + filter (client-side demo)
      ...
      <div className="bg-white rounded-xl shadow-xl p-4 flex flex-col gap-2 min-h-[600px]">
        <div className="h-[550px] overflow-y-auto rounded-2xl">
          ...
          <div className="w-full flex justify-between items-center my-4">
            {/* Re-use your SearchBar component if it accepts props; otherwise it works as-is */}
            <SearchBar />
          ...
          {/* TABLE: first 3 columns => ~40% width (15% + 15% + 10%) */}
          <table className="min-w-full text-left border-collapse">
            <thead className="sticky top-0 bg-[#F9F4FF]">
              <tr>
                ...
                {/* remaining 5 columns share remaining width */}
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full overflow-hidden bg-gray-100">
                        {/* fallback avatar; ensure these paths exist in your public folder */}
                        <Image
                  ...
                  <td className="py-3 px-4">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-gray-500">{/* optional secondary info */}</span>
                    </div>
                  </td>

                  {/* Orders count */}
                  <td className="pt-5 px-4 flex flex-row gap-1 text-center "><Image src='/dashboard/bag-tick.png' height={20} width={20} alt='bag'/><p className='pt-0.5'>{c.orders}</p></td>
                  ...
                  <td className="py-3 px-4">
                    ...
                    {/* Inline controllers/modal placeholders: open conditionally */}
